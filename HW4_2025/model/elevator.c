...
typedef struct QHsmTstTag {
	...
	int emergency_status;   /* shows which state of emergency, 1 for up 2 for stragiht down */
	int emergency_calls;
	int emergency_call_time;
	double emergency_total_time;
	int emergency_served;
...
void clearPending_Emergency(void); /*Clear all pending status in care of emergency*/
void clampFloor(void); /*Prevents forever decrementing during emergency when no other calls*/
int getcompletedEm(void);
...
void QHsmTst_ctor(void) {
	...
	HSM_QHsmTst.emergency_status = 0;
	HSM_QHsmTst.emergency_calls = 0;
	HSM_QHsmTst.emergency_call_time = 0; /*service time*/
	HSM_QHsmTst.emergency_total_time = 0; /*cum service time*/
	HSM_QHsmTst.emergency_served = 0;
	for (x=0; x<5; x++){
...
QState QHsmTst_elevator(QHsmTst *me) {
	switch (Q_SIG(me)) {
		...
		case EMERGENCY_KEY:{
			BSP_display("Emergency activated\n");
			return Q_HANDLED();
		}
		case TERMINATE_SIG: {
			BSP_exit();
...
QState QHsmTst_stopped(QHsmTst *me) {
	switch (Q_SIG(me)) {
		case EMERGENCY_KEY: { /*My code*/
			if (EMERGENCY_ON == 0){
				HSM_QHsmTst.emergency_call_time = simTime; /*Reset emergency call time*/
				HSM_QHsmTst.emergency_calls++; /*Increment no of emergency calls*/
				BSP_display("Emergency activated while stopped\n");
				EMERGENCY_ON = 1;
				clearPending_Emergency(); //Clear all floor pends 
				HSM_QHsmTst.floor_req_curr[0] = 1; //set request status for floor 1 maybe redudndant
				updatePending(0); // Set pending status for floor 1
				HSM_QHsmTst.emergency_status = 2; /*Want to move down to floor 1*/
				return Q_TRAN(&QHsmTst_moving); //Force a move to floor 1
			}
		...
		case Q_ENTRY_SIG: {
				...
				/*My code*/
				if (HSM_QHsmTst.emergency_status == 2 && EMERGENCY_ON == 1 && HSM_QHsmTst.curr_floor == 0){ /*If emergency manuever successful*/
					HSM_QHsmTst.emergency_total_time += simTime - HSM_QHsmTst.emergency_call_time;
					EMERGENCY_ON = 0;
					HSM_QHsmTst.emergency_status = 0; /*No more emergency*/
					HSM_QHsmTst.emergency_served++; /*Emergency served counter*/
					BSP_display("Emergency Serviced\n");
				}
				else if (HSM_QHsmTst.emergency_status == 1 && EMERGENCY_ON == 1){ /*We reached floor but in emergency status*/
							HSM_QHsmTst.curr_dir = -1; /*Go downwards*/
							clearPending_Emergency(); /*Clear all pendings*/
							HSM_QHsmTst.floor_req_curr[0] = 1; /*Set request and pending to 1*/
							HSM_QHsmTst.floor_pen[0] = 1;
							HSM_QHsmTst.emergency_status = 2; /*Update emergency status*/
							return Q_TRAN(&QHsmTst_moving); /*Force move down*/
				}
            		return Q_HANDLED();
        		}
        	case Q_EXIT_SIG: {
        	...
        	case TICK_SIG: {
				...
				if (EMERGENCY_ON == 1){
					clearPending_Emergency();
					HSM_QHsmTst.floor_req_curr[0] = 1;
					HSM_QHsmTst.floor_pen[0] = 1;
					HSM_QHsmTst.curr_dir = -1;
					BSP_display("Dont care to stop, emergency\n");
					return Q_TRAN(&QHsmTst_moving);	/*Prevent stopping at floor due to new call*/
				}
	    
	    		if (checkPending() == -1){ /*If the current floor is pending, stop for STOP_TIME_F*/
					STOP_TIME_F = 5 + my_rand() % 10 + 1;
...
QState QHsmTst_moving(QHsmTst *me) {
	switch (Q_SIG(me)) {
		case EMERGENCY_KEY: {
			if (EMERGENCY_ON == 0){
				BSP_display("Emergency activated while moving\n");
				HSM_QHsmTst.emergency_calls++;
				HSM_QHsmTst.emergency_call_time = simTime;
				EMERGENCY_ON = 1;
				if (HSM_QHsmTst.move_time < MOVE_TIME_F-1) HSM_QHsmTst.move_time++;
				else{
					HSM_QHsmTst.move_time = 0;
					HSM_QHsmTst.emergency_status = 1; /*Moving towards next floor*/
					HSM_QHsmTst.curr_floor = HSM_QHsmTst.curr_floor + HSM_QHsmTst.curr_dir; /*Update the current floor*/
					clampFloor();
					if (checkPending() != -1){ /*Basically if not already marked pending*/
						clearPending_Emergency(); /*Clear all pending floors*/
						updatePending(HSM_QHsmTst.curr_floor);
						return Q_TRAN(&QHsmTst_stopped); 
					}
					else return Q_TRAN(&QHsmTst_stopped); /*Just stop at next floor*/
				}
			}
			return Q_HANDLED();
		}
		case Q_ENTRY_SIG: {
        	...
        	case Q_EXIT_SIG: {
					...
					if (EMERGENCY_ON == 0) /*Dont add time and reset when we are rejecting entering STOPPED STATE*/
        	...
        	case TICK_SIG: {
				...
				HSM_QHsmTst.curr_floor = HSM_QHsmTst.curr_floor + HSM_QHsmTst.curr_dir; /*Update the current floor*/
				clampFloor();
				if (EMERGENCY_ON == 1 && HSM_QHsmTst.emergency_status == 2){
					clearPending_Emergency(); /*Clear all pending but next floor to force move*/
					updatePending(HSM_QHsmTst.curr_floor);
					return Q_TRAN(&QHsmTst_stopped);
				}
...
/*In case of emergency, clear all pending status*/
void clearPending_Emergency(void){
	for (int i = 0; i < 5; i++){ 
...
void printData(void){
	int x=0;
	for (x=0; x<5; x++){
		if (HSM_QHsmTst.floor_calls[x] > 0){
	...
	printf("Emergency %d calls\n", HSM_QHsmTst.emergency_calls);
	printf("Average time emergency %f\n", HSM_QHsmTst.emergency_total_time/HSM_QHsmTst.emergency_calls);
	printf("Calls serviced %d\n", HSM_QHsmTst.emergency_served);
	return;
}

void debugPrintState(void) {
    printf("DEBUG: curr_floor=%d curr_dir=%d emergency_status=%d EMERGENCY_ON=%d tottime %d\n",
           HSM_QHsmTst.curr_floor,
           HSM_QHsmTst.curr_dir,
           HSM_QHsmTst.emergency_status,
           EMERGENCY_ON,
			HSM_QHsmTst.emergency_total_time);
}

void clampFloor(void) {
    if (HSM_QHsmTst.curr_floor < 0)
        HSM_QHsmTst.curr_floor = 0;
    if (HSM_QHsmTst.curr_floor > 4)
        HSM_QHsmTst.curr_floor = 4;
}

int getcompletedEm(void){
	return HSM_QHsmTst.emergency_served;
}
